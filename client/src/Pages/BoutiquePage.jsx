...
function BoutiquePage() {
  ...
  const [selectedBoutiqueBanner, setSelectedBoutiqueBanner] = useState(null);
  const [searchParams, setSearchParams] = useSearchParams();
  const dispatch = useDispatch();
  useEffect(()=>{
    const getBoutiqueBanners = async () => {
      try {
        const response = await axiosInstance.get(import.meta.env.VITE_API_URL + "/boutique/all-boutique-banners");
